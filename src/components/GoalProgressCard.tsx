    ...
    return (
      <section className="rounded border border-slate-200 bg-white p-6 text-sm text-slate-700">
        <header className="flex items-start justify-between">
          <div>
            <p className="text-xs uppercase tracking-wide text-slate-400">Performance Goal</p>
            <h2 className="mt-1 text-base font-semibold text-slate-900">{month.label} 성과 목표 없음</h2>
          </div>
        </header>
        ...
        <div className="mt-6 flex flex-wrap items-center justify-between gap-4">
          <div>
            <p className="text-xs text-slate-500">이번 달 손익</p>
            <p className="text-lg font-semibold text-slate-900">{formatCurrency(achievedAmount)}</p>
          </div>
          <Link
            to="/settings"
            className="inline-flex items-center rounded border border-slate-300 px-3 py-2 text-xs font-semibold text-slate-700 transition hover:bg-slate-100"
          >
            목표 설정하기
          </Link>
        </div>
  ...
  return (
    <section className="rounded border border-slate-200 bg-white p-6 text-sm text-slate-700">
      <header className="flex items-start justify-between">
        <div>
          <p className="text-xs uppercase tracking-wide text-slate-400">Performance Goal</p>
          <h2 className="mt-1 text-base font-semibold text-slate-900">{month.label} 목표 진행률</h2>
        </div>
        <Link
          to="/settings"
      ...
      <div className="mt-5 space-y-4">
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          <div>
            ...
            <p className="mt-1 text-sm font-semibold text-slate-900">{formatCurrency(goal.targetAmount)}</p>
          </div>
          <div>
            <p className="text-xs text-slate-500">누적 손익</p>
            <p className="mt-1 text-sm font-semibold text-slate-900">{formatCurrency(achievedAmount)}</p>
          </div>
          <div>
