...
const SeedOverviewChart: React.FC<SeedOverviewChartProps> = ({ initialSeed, trades }) => {
  ...
  const data = useMemo<ChartPoint[]>(() => {
    if (initialSeed === null || Number.isNaN(initialSeed)) {
      return [];
    }
  ...
  return (
    <div className="rounded border border-slate-200 bg-white p-6 dark:border-slate-800 dark:bg-slate-900">
      ...
      {initialSeed === null || Number.isNaN(initialSeed) ? (
        <div className="py-10 text-center text-xs text-slate-500 dark:text-slate-400">
          초기 자본 정보가 없어 자본 추이를 표시할 수 없습니다.
        </div>
      ) : data.length <= 1 ? (
        <div className="py-10 text-center text-xs text-slate-500 dark:text-slate-400">
          아직 거래가 없습니다. 첫 거래를 기록하면 자본 추이 그래프가 생성됩니다.
        </div>
      ) : (
