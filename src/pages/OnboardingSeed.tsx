  ...
  if (!user) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-slate-50 px-4 text-slate-900 dark:bg-slate-950 dark:text-slate-100">
        <div className="w-full max-w-sm space-y-8 rounded-3xl border border-slate-200 bg-white p-10 shadow-2xl dark:border-slate-800 dark:bg-slate-900">
          <TrakkoAuthHero />
          <div className="space-y-3">
            <button
              type="button"
              onClick={handleLogin}
              className="flex w-full items-center justify-center gap-2 rounded-lg bg-slate-900 px-4 py-2 text-sm font-semibold text-white transition hover:bg-slate-800 dark:bg-slate-100 dark:text-slate-900 dark:hover:bg-slate-200"
            >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <section className="space-y-3">
            <header className="text-left">
              ...
              <h2 className="text-sm font-semibold text-slate-900 dark:text-slate-100">금액을 어떤 통화로 관리할까요?</h2>
              <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
            ...
            {preferencesLoading && (
              <p className="text-xs text-slate-400 dark:text-slate-500">표시 통화를 불러오는 중입니다...</p>
            )}
            {preferencesError && (
          ...
          {isValid && (
            <p className="text-xs text-slate-500 dark:text-slate-400">표시 미리보기: {formatCurrency(parsedSeed)}</p>
          )}
          {touched && !isValid && (
